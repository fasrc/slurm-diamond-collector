...
A collector for SLURM sshare statistics.
"""
...
class SlurmSshareCollector(diamond.collector.Collector):
    ...
    def collect(self):
        """
        Collect the SLURM sshare stats
        """
        try:
            ...
            self.log.exception("failed fetching SLURM sshare statistics")
            return
        ...
        for line in proc.stdout:
            ...
            if User and Account:
                self.publish('{}.{}.rawshares'.format(Account, User),
                             RawShares)
                self.publish('{}.{}.normshares'.format(Account, User),
                             NormShares, precision=6)
                self.publish('{}.{}.rawusage'.format(Account, User), RawUsage)
                self.publish('{}.{}.normusage'.format(Account, User),
                             NormUsage, precision=6)
                self.publish('{}.{}.fairshare'.format(Account, User),
                             Fairshare, precision=6)
