...
class SlurmClusterStatusCollector(diamond.collector.Collector):
    ...
    def collect(self):
        """
        Collect SLURM cluster status metrics
        """
        try:
            ...
            self.log.exception("error occured fetching cluster status metrics")
            return
        ...
        for line in proc.stdout:
            ...
            # Slurm only lists actual free memory so we have to back calculate
            # how much is actually used.
            if node['FreeMem'] != 'N/A':
                MemLoad = MemLoad + (int(node['RealMemory']) -
                                     int(node['FreeMem']))

            # Count how many nodes are in each state
            if node['State'] in ['IDLE', 'IDLE+COMPLETING']:
                IDLETot = IDLETot + 1
                IDLECPU = IDLECPU + int(node['CPUTot'])
                IDLEMem = IDLEMem + int(node['RealMemory'])

            if node['State'] in ['MIXED', 'MIXED+COMPLETING']:
                MIXEDTot = MIXEDTot + 1
                MIXEDCPU = MIXEDCPU + int(node['CPUTot'])
                MIXEDMem = MIXEDMem + int(node['RealMemory'])

            if node['State'] in ['ALLOCATED', 'ALLOCATED+COMPLETING']:
                ALLOCTot = ALLOCTot + 1
            ...
            if "DRAIN" in node['State'] and node['State'] not in ['IDLE+DRAIN',
                                                                  'DOWN+DRAIN']:
                DRAINTot = DRAINTot + 1
            ...
            # Calculate percent occupation of all nodes. Some nodes may have
            # few cores used but all their memory allocated.
            # Thus the node is fully used even though it is not labelled Alloc.
            # This metric is an attempt to count this properly.
            PerAlloc = PerAlloc + max(
                float(node['CPUAlloc']) / float(node['CPUTot']),
                float(node['AllocMem']) / float(node['RealMemory'])
            )
