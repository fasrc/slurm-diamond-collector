...
class SlurmSchedStatsCollector(diamond.collector.Collector):
    ...
    def collect(self):
        ...
        try:
            ...
            self.publish('bf_mean_cycle', (sd['bMeancycle'] if bMeancycle in sd else 0))
            self.publish('bf_depth_mean', (sd['bDepthMean'] if bDepthMean in sd else 0))
            self.publish('bf_depth_mean_try', (sd['bDepthMeantrydepth'] if bDepthMeantrydepth in sd else 0))
            self.publish('bf_queue_length_mean', (sd['bQueuelengthmean'] if bQueuelengthmean in sd else 0))
