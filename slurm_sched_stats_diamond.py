...
class SlurmSchedStatsCollector(diamond.collector.Collector):
    ...
    def collect(self):
        ...
        try:
            ...
            self.publish('bf_mean_cycle', (sd['bMeancycle'] if sd.has_key('bMeancycle') else 0))
            self.publish('bf_depth_mean', (sd['bDepthMean'] if sd.has_key('bDepthMean') else 0))
            self.publish('bf_depth_mean_try', (sd['bDepthMeantrydepth'] if sd.has_key('bDepthMeantrydepth') else 0))
            self.publish('bf_queue_length_mean', (sd['bQueuelengthmean'] if sd.has_key('bQueuelengthmean') else 0))
