...
class SlurmSchedStatsCollector(diamond.collector.Collector):
    ...
    def collect(self):
        try:
            ...
            self.log.exception("error collecting statistics from pyslurm")
            return

        schedule_cycle_counter = sdiag.get("schedule_cycle_counter")
        ...
        self.publish('main_total_cycles', schedule_cycle_counter)

        if schedule_cycle_counter > 0:
            self.publish(
                'main_mean_cycle',
                sdiag.get("schedule_cycle_sum") / schedule_cycle_counter
            )
            self.publish('main_mean_depth_cycle', (
                sdiag.get("schedule_cycle_depth") / schedule_cycle_counter
            ))
