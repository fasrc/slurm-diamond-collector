...
class SlurmSchedStatsCollector(diamond.collector.Collector):
    ...
    def collect(self):
        ...
        try:
            ...
            self.publish('bf_mean_cycle', (if bMeancycle in sd sd['bMeancycle'] else 0))
            self.publish('bf_depth_mean', (if bDepthMean in sd sd['bDepthMean'] else 0))
            self.publish('bf_depth_mean_try', (if bDepthMeantrydepth in sd sd['bDepthMeantrydepth'] else 0))
            self.publish('bf_queue_length_mean', (if bQueuelengthmean in sd sd['bQueuelengthmean'] else 0))
